...
public class ScoreCalculator {
    ...
    private static final int[][] TABLE_DEALER_TUMO = {
            { 0, 700, 1300, 2600 }, // 20符
    ...
    private static final int[][][] TABLE_CHILd_TUMO = {
    ...
    private static final int[] TABLE_DEALER_PAIRS_TUMO = { 0, 700, 1300, 2600 };

    // 七対子のときの子のツモ点数表 1翻 2翻 3翻 4翻
    private static final int[][] TABLE_CHILD_PAIRS_TUMO = {
            { 0, 0, 800, 1600 }, // 子が払う点数
            { 0, 0, 1600, 3200 } // 親が払う点数
    };

    // 親のロン点数表 1翻 2翻 3翻 4翻
    private static final int[][] TABLE_DEALER_RON = {
    ...
    // 子のロン点数表 1翻 2翻 3翻 4翻
    private static final int[][] TABLE_CHILd_RON = {
    ...
    private static final int[] TABLE_DEALER_PAIRS_RON = { 0, 2400, 4800, 9600 };

    // 七対子のときの子のロン点数表 1翻 2翻 3翻 4翻
    private static final int[] TABLE_CHILD_PAIRS_RON = { 0, 1600, 3200, 6400 };
    ...
    public static int[] calTumoScore(boolean isDealer, int han, int mark) {
        ...
        if (han <= 4) {
            if (mark == 25) { // 七対子のときのみ処理を分ける
                if (isDealer) {
                    pay[0] = 0;
                    pay[1] = TABLE_DEALER_PAIRS_TUMO[han - 1];
                } else {
                    pay[0] = TABLE_CHILD_PAIRS_TUMO[1][han - 1]; // 親が払う点数を更新
                    pay[1] = TABLE_CHILD_PAIRS_TUMO[0][han - 1]; // 子が払う点数を更新
                }
            } else {
                int n = (mark - 20) / 10; // 表の何行目か
                if (isDealer) {
                    pay[0] = 0; // 親が払う点数を更新
                    pay[1] = TABLE_DEALER_TUMO[n][han - 1]; // 子が払う点数を更新
                } else {
                    pay[0] = TABLE_CHILd_TUMO[n][1][han - 1]; // 親が払う点数を更新
                    pay[1] = TABLE_CHILd_TUMO[n][0][han - 1]; // 子が払う点数を更新
                }
            }
        }
    ...
    public static int calRonScore(boolean isDealer, int han, int mark) {
        ...
        if (han <= 4) {
            if (mark == 25) {
                if (isDealer) {
                    return TABLE_DEALER_PAIRS_RON[han - 1];
                } else {
                    return TABLE_CHILD_PAIRS_RON[han - 1];
                }
            } else {
                int n = (mark - 30) / 10; // 表の何行目かを格納
                if (isDealer) {
                    return TABLE_DEALER_RON[n][han - 1];
                } else {
                    return TABLE_CHILd_RON[n][han - 1];
                }
            }
        }
